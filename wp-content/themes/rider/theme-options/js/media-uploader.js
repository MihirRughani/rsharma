(function($) {
	$(document).ready(function() {
		...
		function rider_add_file(event, selector) {
			...
			frame.on( 'select', function() {
				...
				selector.find('.remove-image, .remove-file').on('click', function() {
					rider_remove_file( $(this).parents('.section') );
				});
			});
		...
		function rider_remove_file(selector) {
			...
			selector.find('.upload-button').live('click', function() {
				rider_add_file(event, $(this).parents('.section'));
			});
		}
